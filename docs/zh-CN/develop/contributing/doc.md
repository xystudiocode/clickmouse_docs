...
你可以使用`git clone https://github.com/username/clickmouse_docs.git`下载源码到本地
...
并创建一个remote链接：
```bash
git remote add origin https://github.com/username/clickmouse_docs.git
```
...
#### 提交PR
你需要在[此链接](https://github.com/xystudiocode/clickmouse_docs)上创建一个Pull request，然后等待review。
...
运行`npm run build`编译脚本，可以放到`/clickmouse_docs/`目录来部署。
